...
function onYouTubeIframeAPIReady() {
  ...
  for ( var i = 0; i < ypi_iframes_len; i++ ) {
    if ( ypi_iframes[i].src.indexOf( "youtube.com" ) > -1 ) {
      ...
      // If URL of video is http, change to https
      if ( ypi_iframes[i].src.match( '^http://' ) )
        ypi_iframes[i].src = ypi_iframes[i].src.replace( /^http:\/\//i, 'https://' );
...
function onPlayerStateChange( event ) {
  ...
  // If a video has stopped playing
    ...
    for ( var k = 0; k < ypi_player_count; k++ ) {
      if ( ypi_players[k].id == stopped_video_id ) {
        ypi_players[k].playing = false;
        if ( event.data == YT.PlayerState.PAUSED )
          ypi_players[k].paused = true;
        else
          ypi_players[k].paused = false;
      }
    }

    // If all videos have stopped playing
    for ( var l = 0; l < ypi_player_count; l++) {
      if ( ypi_players[l].playing === true )
        break;
      if ( l == ypi_player_count - 1 ) {
        ...
        if ( ypi_pause_icon == 'on' && event.data == YT.PlayerState.PAUSED ) {
          ...
          for ( var m = 0; m < ypi_player_count; m++ ) {
            if ( ypi_players[m].paused === true ) {
